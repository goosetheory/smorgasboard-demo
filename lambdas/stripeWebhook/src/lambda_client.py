...
class LambdaClient:
	...
	def send_purchase_email(self, cognito_username):
		try:
			payload = {
				'emailType': EmailType.ON_PAY_FOR_BOARD.value,
				'cognitoUsername': str(cognito_username)
			}

			logger.info(f'Sending purchased email')
			...
			logger.info('Successfully sent purchase email.')
			return True
