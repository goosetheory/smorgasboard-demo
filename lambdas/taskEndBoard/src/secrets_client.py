...
class SecretsClient:
	def __init__(self):
		self.archive_function_secret_name = os.environ.get('ENV') + '/ArchiveInfo'
		self.email_info_secret_name = os.environ.get('ENV') + '/SendEmailInfo'
		self.region_name = os.environ.get('AWS_REGION')
		self.secrets_client = None
		self.archive_function_name = None
		self.send_email_function_name = None

	def get_archive_function_name(self):
		if not self.archive_function_name:
			secret = self._get_secret(self.archive_function_secret_name)
			self.archive_function_name = secret['archive_function_name']
		return self.archive_function_name
